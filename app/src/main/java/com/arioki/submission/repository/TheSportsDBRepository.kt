...
class TheSportsDBRepository(private val api: TheSportsDBApi.Api) {
    ...
    fun searchPlayers(value: String, callback: LookupPlayersRepositoryCallback) {
        api.searchPlayers(value).enqueue(object : Callback<LookupAllPlayersResponse?> {
            override fun onFailure(call: Call<LookupAllPlayersResponse?>, t: Throwable) {
                callback.onError(t)
            }
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    val result = response.body()?.player
                        ?.filter { it -> it.strSport == "Soccer" }
                        ?.map {
                            with(it) {
                                LookupAllPlayerItem(
